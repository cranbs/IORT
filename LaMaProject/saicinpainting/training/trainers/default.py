...
from LaMaProject.saicinpainting.training.data.datasets import make_constant_area_crop_params
from LaMaProject.saicinpainting.training.losses.distance_weighting import make_mask_distance_weighter
from LaMaProject.saicinpainting.training.losses.feature_matching import feature_matching_loss, masked_l1_loss
from LaMaProject.saicinpainting.training.modules.fake_fakes import FakeFakesGenerator
from LaMaProject.saicinpainting.training.trainers.base import BaseInpaintingTrainingModule, make_multiscale_noise
from LaMaProject.saicinpainting.utils import add_prefix_to_keys, get_ramp
...
class DefaultInpaintingTrainingModule(BaseInpaintingTrainingModule):
    ...
    def forward(self, batch):
        ...
        if pred.shape[2:] != img.shape[2:]:
            pred = torch.nn.functional.interpolate(pred.float(), size=img.shape[2:], mode="nearest")
        batch['inpainted'] = mask * pred + (1 - mask) * img
